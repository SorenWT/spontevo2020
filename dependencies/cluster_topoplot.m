function cluster_topoplot(data,chanlocs,sig,sigmask,size)
% 
% if size(sig,1) == 1
%    sig = sig';
% end
...
if nargs > 2
    ...
    for i = 1:2:length(varargin)
        ...
        switch Param
            ...
            case 'emarker3' %added by SWT on 24/03/2019 to allow two different subsets of electrode markers
                if ~iscell(Value) | length(Value) > 5
...
if ~strcmpi(STYLE,'grid')                     % if not plot grid only
    ...
    if ~isempty(EMARKER3CHANS)
                [tmpa tmpa mark3chans] = intersect_bc(EMARKER3CHANS,pltchans);
    end
