...
p(2,2,1).select();
nicecorrplot(nanmean(allmeas{1}.naerpindex,1),nanmean(allmeas{1}.ttvindex,1),{'Pseudotrial method','TTV method'});
FixAxes(gca,16)
...
%     H = sigstar({get(p(1,c,1).axis,'XLim')},min(compstats.erspdiff{c}.prob),0,18)
%     delete(H(1)); pos = get(H(2),'position'); yl = get(gca,'YLim'); set(H(2),'position',[pos(1) yl(2)-0.05*diff(yl) pos(3)])
%     
%     p(2,c,1).select()
%     %Plot_sigmask(p(2,c,1).axis,compstats.erspdiff{c}.mask,'cmapline','LineWidth',5)
%     H = sigstar({get(p(2,c,1).axis,'XLim')},min(compstats.erspdiff{c}.prob),0,18)
%     delete(H(1)); pos = get(H(2),'position'); yl = get(gca,'YLim'); set(H(2),'position',[pos(1) yl(2)-0.05*diff(yl) pos(3)])
% end
% 
% p.de.margin = [5 5 5 5];
...
for c = 1:settings.nfreqs
    ...
    H = sigstar({get(p(1,c,1).axis,'XLim')},min(min(alloutputs_osci.ersp.pt.stats{c}.prob)),0,18)
    delete(H(1)); pos = get(H(2),'position'); yl = get(gca,'YLim'); set(H(2),'position',[pos(1) yl(2)-0.05*diff(yl) pos(3)])
end
...
for c = 1:settings.nfreqs
    ...
    H = sigstar({get(p(2,c,1).axis,'XLim')},min(min(alloutputs_frac.ersp.pt.stats{c}.prob)),0,18);
    delete(H(1)); pos = get(H(2),'position'); yl = get(gca,'YLim'); set(H(2),'position',[pos(1) yl(2)-0.05*diff(yl) pos(3)]);
end
...
for c = 1:settings.nfreqs
    osci_pt_diff_stats{c} = EasyClusterCorrect({permute(squeeze(allmeas_osci{c}.ersp.real),[1 3 2]),...
        zeros(size(permute(squeeze(allmeas_osci{c}.ersp.real),[1 3 2])))},...
        settings.datasetinfo,'ft_statfun_fast_signrank',opts);
    frac_pt_diff_stats{c} = EasyClusterCorrect({permute(squeeze(allmeas_frac{c}.ersp.real),[1 3 2]),...
        zeros(size(permute(squeeze(allmeas_frac{c}.ersp.real),[1 3 2])))},...
        settings.datasetinfo,'ft_statfun_fast_signrank',opts);
end
...
p.pack('v',{1/2 1/2})
p(1).pack('h',repmat({1/settings_osci.nfreqs},settings_osci.nfreqs,1)')
p(2).pack('h',repmat({1/settings_osci.nfreqs},settings_osci.nfreqs,1)')
