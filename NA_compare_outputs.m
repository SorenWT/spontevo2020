...
%     ylabelunits(settings)
%     
%     plotindx = round(linspace(0,max(settings.aucindex),5));
%     plotindx(1) = [];
%     for cc = 1:4
%         p(2,c,cc+1).select()
%         %axes(p(2,c,cc+1).axis)
%         plotdata = squeeze(nanmean(allmeas1{c}.ttversp.real(:,plotindx(cc),:),3)-nanmean(allmeas2{c}.ttversp.real(:,plotindx(cc),:),3));
%         if strcmpi(settings.datatype,'MEG')
%             ft_cluster_topoplot(settings.layout,plotdata,settings.datasetinfo.label,...
